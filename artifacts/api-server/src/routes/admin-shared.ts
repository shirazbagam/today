...
import { emitNotificationNew } from "../lib/socketio.js";
...
let _adminAccountsUsernameMigrated = false;
export async function ensureAdminAccountsUsernameColumn() {
  if (_adminAccountsUsernameMigrated) return;
  try {
    await db.execute(sql`ALTER TABLE admin_accounts ADD COLUMN IF NOT EXISTS username TEXT UNIQUE`);
  } catch { /* column likely already exists or the table does not yet contain this row */ }
  _adminAccountsUsernameMigrated = true;
}
...
export function getAdminSecret(): string {
  ...
  if (!s || s.length < 8) {
    logger.fatal("[AUTH] ADMIN_SECRET environment variable must be set and ≥8 characters.");
    process.exit(1);
  }
  return s;
}
...
export async function sendUserNotification(userId: string, title: string, body: string, type: string, icon: string) {
  ...
  emitNotificationNew(userId, { id, title, body, type, icon });
