...
- **Rider Service Preference:** Rider profiles store `serviceType` (`both`, `deliveries`, or `rides`). The rider app lets riders choose which work types they accept, and `/rider/requests` filters delivery orders and ride requests server-side from that saved preference.
...
- **Order lifecycle audit:** `order_status_history` records status changes from customer, vendor, rider, admin, and system flows. Realtime events now include named `order:status`, `order:out_for_delivery`, `order:delivered`, `ride:status`, `rider:arrived`, `ride:started`, `ride:completed`, and `notification:new`.
- **Support/dispute tickets:** `support_tickets` stores order-linked support, dispute, refund, delivery, and quality tickets with admin listing and status updates.
- **Shared API Transport:** Admin, rider, and vendor web apps delegate HTTP requests to the shared API client transport while preserving role-specific auth headers and logout behavior.

**Database Schema Highlights:**
- Key tables: `usersTable`, `riderProfilesTable`, `magicLinkTokensTable`, `productsTable`, `ordersTable`, `orderStatusHistoryTable`, `supportTicketsTable`, `walletTransactionsTable`, `ridesTable`, `rideBidsTable`, `liveLocationsTable`, `rideRatingsTable`, `riderPenaltiesTable`, `popularLocationsTable`, `schoolRoutesTable`, `schoolSubscriptionsTable`, `productVariantsTable`, `bannersTable`, `userInteractionsTable`, `delivery_whitelist`, `delivery_access_requests`, `system_audit_log`, `categories`, `kyc_verifications`, `location_logs`, `vanRoutesTable`, `vanVehiclesTable`, `vanSchedulesTable`, `vanBookingsTable`, `rideMessagesTable`.
...
### Recent Bug Fixes (Promos & Environment)

1. **Vendor-scoped promo codes:** Promo codes can now be reused by different vendors while global admin promo codes remain global. Order placement validates vendor-specific promos against the resolved order vendor and falls back to global promos only when appropriate.
2. **Vendor promo creation:** Vendor promo creation now accepts both backend fields (`discountPct`/`discountFlat`) and vendor-app fields (`discountType`/`discountValue`).
3. **Environment template:** `.env.example` now includes required auth, webhook, email, web-push, and app URL secrets.
