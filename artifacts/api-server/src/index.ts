...
import { ensureAdminAccountsUsernameColumn, ensureAuthMethodColumn, ensureRideBidsMigration, ensureOrdersGpsColumns, ensureIdempotencyTable, ensureWalletNormalizedTxId, ensureTwoFactorEnforcedAt, ensureSilenceModeColumns, ensureDefaultServiceZones, ensureDefaultPaymentMethods, ensureOtpSettings, ensureProfileCompleteColumn, ensureOrdersItemsNullable, ensureVendorAutoConfirmColumn, ensureLocationHierarchyTable } from "./routes/admin.js";
import { initVapid } from "./lib/webpush.js";
import { db } from "@workspace/db";
import { getPlatformSettings } from "./routes/admin.js";
import { locationLogsTable, pendingOtpsTable, ordersTable, orderStatusHistoryTable, notificationsTable, usersTable, walletTransactionsTable } from "@workspace/db/schema";
import { lt, eq, and, lte, sql } from "drizzle-orm";
import { getIO, emitOrderStatusEvent, emitNotificationNew } from "./lib/socketio.js";
...
cron.schedule("* * * * *", async () => {
  try {
    ...
    const expired = await db
      .select({ id: ordersTable.id, userId: ordersTable.userId, vendorId: ordersTable.vendorId, riderId: ordersTable.riderId, paymentMethod: ordersTable.paymentMethod, total: ordersTable.total })
      .from(ordersTable)
    ...
    for (const order of expired) {
      await db.transaction(async (tx) => {
        ...
        await tx.insert(notificationsTable).values({ id: notifId, userId: order.userId, title: "Order Cancelled", body: "Your order was cancelled because no vendor accepted it in time. If paid, refund has been processed.", type: "order", icon: "close-circle-outline" });
        emitNotificationNew(order.userId, { id: notifId, title: "Order Cancelled", body: "Your order was cancelled because no vendor accepted it in time. If paid, refund has been processed.", type: "order", icon: "close-circle-outline" });
      });
      if (io) {
...
ensureAuthMethodColumn()
  .then(() => ensureAdminAccountsUsernameColumn())
