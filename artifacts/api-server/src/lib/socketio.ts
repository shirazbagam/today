...
export function emitOrderStatusEvent(payload: {
  id: string;
  userId: string;
  vendorId?: string | null;
  riderId?: string | null;
  ...
  if (!_io) return;
  _io.to("admin-fleet").emit("order:status", payload);
  _io.to(`user:${payload.userId}`).emit("order:status", payload);
  if (payload.vendorId) _io.to(`vendor:${payload.vendorId}`).emit("order:status", payload);
  if (payload.riderId) _io.to(`rider:${payload.riderId}`).emit("order:status", payload);
  if (payload.status === "out_for_delivery") {
    _io.to(`user:${payload.userId}`).emit("order:out_for_delivery", payload);
  }
  if (payload.status === "delivered") {
    _io.to(`user:${payload.userId}`).emit("order:delivered", payload);
  }
}

export function emitRideLifecycleEvent(payload: {
  id: string;
  userId: string;
  riderId?: string | null;
  status: string;
  ...
  if (!_io) return;
  _io.to("admin-fleet").emit("ride:status", payload);
  _io.to(`user:${payload.userId}`).to(`ride:${payload.id}`).emit("ride:status", payload);
  if (payload.riderId) _io.to(`rider:${payload.riderId}`).emit("ride:status", payload);
  if (payload.status === "arrived") _io.to(`user:${payload.userId}`).to(`ride:${payload.id}`).emit("rider:arrived", payload);
  if (payload.status === "in_transit") _io.to(`user:${payload.userId}`).to(`ride:${payload.id}`).emit("ride:started", payload);
  if (payload.status === "completed") _io.to(`user:${payload.userId}`).to(`ride:${payload.id}`).emit("ride:completed", payload);
}

export function emitNotificationNew(userId: string, payload: {
  id?: string;
  title: string;
  body: string;
  type?: string;
