...
export async function apiFetch(path: string, opts: RequestInit = {}, _retryBudget = 2, _returnEnvelope = false): Promise<any> {
  ...
  try {
    return await customFetch(path, {
      ...
      responseType: "json",
      unwrapEnvelope: !_returnEnvelope,
    });
  } catch (error) {
    if (error instanceof ApiError) {
      const err = (error.data ?? {}) as ApiEnvelope & Record<string, unknown>;
      if (error.status === 401) {
      ...
      if (error.status === 403) {
        ...
        const rejectionReason = err.rejectionReason ?? nested?.rejectionReason ?? null;
        const approvalStatus = err.approvalStatus ?? nested?.approvalStatus ?? null;
        ...
        const isAuthDenial =
          ...
          AUTH_DENY_PHRASES.some(p => String(msg).toLowerCase().startsWith(p));
        if (isAuthDenial) {
          triggerLogout("access_denied");
        }
        throw Object.assign(new Error(String(msg) || "Access denied"), { status: 403, code, rejectionReason, approvalStatus });
      }
      const message = err.error || err.message || error.message || "Request failed";
      throw Object.assign(new Error(String(message)), { responseData: err, status: error.status });
    }
