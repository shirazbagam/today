...
import { usersTable, riderProfilesTable, vendorProfilesTable, ordersTable, orderStatusHistoryTable, rideBidsTable, ridesTable, riderPenaltiesTable, walletTransactionsTable, notificationsTable, liveLocationsTable, reviewsTable, rideRatingsTable, locationLogsTable, rideServiceTypesTable } from "@workspace/db/schema";
...
import { emitRiderLocation, emitRiderStatus, emitOrderStatusEvent, emitRideDispatchUpdate, emitRideLifecycleEvent, emitRideOtp, emitNotificationNew, getIO } from "../lib/socketio.js";
...
const profileSchema = z.object({
  ...
  serviceType: z.enum(["both", "deliveries", "rides"]).optional(),
...
router.get("/me", async (req, res) => {
  ...
  const [riderProfile] = await db.select({
    ...
    serviceType: riderProfilesTable.serviceType,
  ...
  sendSuccess(res, {
    ...
    vehicleType: riderProfile?.vehicleType ?? user.vehicleType, serviceType: riderProfile?.serviceType ?? "both", vehiclePlate: user.vehiclePlate,
...
router.patch("/profile", async (req, res) => {
  ...
  const { name, email, cnic, address, city, emergencyContact, vehicleType, serviceType, vehiclePlate, vehicleRegNo, drivingLicense, bankName, bankAccount, bankAccountTitle, avatar, cnicDocUrl, licenseDocUrl, regDocUrl, vehiclePhoto } = parsed.data;
  ...
  if (serviceType      !== undefined) profileUpdates.serviceType = serviceType;
  ...
  let user: typeof usersTable.$inferSelect & { vehicleType?: string | null; serviceType?: string | null };
  try {
    ...
    if (Object.keys(profileUpdates).length > 1) {
      ...
      const [rp] = await db.select({ vehicleType: riderProfilesTable.vehicleType, serviceType: riderProfilesTable.serviceType })
        .from(riderProfilesTable).where(eq(riderProfilesTable.userId, riderId)).limit(1);
      user = { ...user, vehicleType: rp?.vehicleType ?? null, serviceType: rp?.serviceType ?? "both" };
    } else {
      const [rp] = await db.select({ vehicleType: riderProfilesTable.vehicleType, serviceType: riderProfilesTable.serviceType })
        .from(riderProfilesTable).where(eq(riderProfilesTable.userId, riderId)).limit(1);
      user = { ...user, vehicleType: rp?.vehicleType ?? null, serviceType: rp?.serviceType ?? "both" };
  ...
  sendSuccess(res, {
    ...
    cnic: user.cnic, address: user.address, city: user.city, area: user.area,
    emergencyContact: user.emergencyContact,
    vehicleType: user.vehicleType, serviceType: user.serviceType ?? "both", vehiclePlate: user.vehiclePlate,
    vehicleRegNo: user.vehicleRegNo, drivingLicense: user.drivingLicense,
    bankName: user.bankName, bankAccount: user.bankAccount, bankAccountTitle: user.bankAccountTitle,
    accountLevel: user.accountLevel, kycStatus: user.kycStatus,
...
router.get("/requests", async (req, res) => {
  ...
  const [orders, rides, myBids, riderLoc, profileRows] = await Promise.all([
  ...
  const riderProfile = profileRows[0];
  const riderVehicle = normalizeVehicleType(String(riderProfile?.vehicleType ?? riderUser.vehicleType ?? ""));
  const serviceType = riderProfile?.serviceType ?? "both";
  ...
  res.status(200).json({
    ...
    data: {
      orders: serviceType === "rides" ? [] : orders.map(o => ({ ...o, total: safeNum(o.total) })),
      rides: serviceType === "deliveries" ? [] : maskedRides,
    },
  });
});
...
async function updateRiderOrderStatus(req: Request, res: Response) {
  ...
  if (status === "cancelled") {
    ...
    await db.insert(orderStatusHistoryTable).values({
      id: generateId(), orderId: order.id, previousStatus: order.status, status: "preparing",
      changedByUserId: riderId, changedByRole: "rider", note: "cancelled_by_rider",
    }).catch((err: Error) => { logger.error("[rider] status history failed:", err.message); });
    const riderChangeLang = await getUserLanguage(order.userId);
    const notifId = generateId();
    const notifTitle = t("notifRiderChange", riderChangeLang) + " 🔄";
    const notifBody = t("notifRiderChangeBody", riderChangeLang);
    await db.insert(notificationsTable).values({
      id: notifId, userId: order.userId,
      title: notifTitle, body: notifBody,
      type: "order", icon: "refresh-outline",
    }).catch((err: Error) => { logger.error("[rider] background op failed:", err.message); });
    emitNotificationNew(order.userId, { id: notifId, title: notifTitle, body: notifBody, type: "order", icon: "refresh-outline" });
    emitOrderStatusEvent({ id: order.id, userId: order.userId, vendorId: order.vendorId, riderId, status: "preparing", previousStatus: order.status, updatedAt: cancelled?.updatedAt instanceof Date ? cancelled.updatedAt.toISOString() : undefined });
    sendSuccess(res, {
  ...
  if (status === "delivered") {
    ...
    if (isCash) {
      ...
      const txResult = await db.transaction(async (tx) => {
        ...
        await tx.insert(orderStatusHistoryTable).values({
          id: generateId(), orderId: order.id, previousStatus: order.status, status,
          changedByUserId: riderId, changedByRole: "rider",
        });
        await tx.insert(walletTransactionsTable).values({
    ...
    const row = await db.transaction(async (tx) => {
      const [updatedOrder] = await tx.update(ordersTable).set(updateData).where(and(eq(ordersTable.id, req.params["id"]!), eq(ordersTable.riderId, riderId), eq(ordersTable.status, order.status))).returning();
...
router.patch("/orders/:id/status", updateRiderOrderStatus);
router.post("/orders/:id/complete", (req, res) => {
  req.body = { ...(req.body ?? {}), status: req.body?.status === "failed" ? "cancelled" : "delivered" };
  return updateRiderOrderStatus(req, res);
});
