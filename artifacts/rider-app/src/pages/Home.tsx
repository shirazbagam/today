...
export default function Home() {
  ...
  const serviceType = user?.serviceType || "both";
  ...
  const serviceTypeMut = useMutation({
    mutationFn: (nextServiceType: "both" | "deliveries" | "rides") =>
      api.updateProfile({ serviceType: nextServiceType }),
    onSuccess: async (_data, nextServiceType) => {
      await refreshUser();
      setDismissed(new Set());
      clearAllDismissed();
      qc.invalidateQueries({ queryKey: ["rider-requests"] });
      showToast(
        nextServiceType === "both"
          ? "Showing deliveries and rides."
          : nextServiceType === "deliveries"
            ? "Showing delivery orders only."
            : "Showing ride requests only.",
        "success",
      );
    },
    onError: (e: any) => {
      showToast(e.message || "Could not update service preference", "error");
    },
  });
      ...
      <main className="px-3 sm:px-4 pt-4 space-y-3 relative z-10 w-full max-w-2xl mx-auto pb-6">
        ...
        {user?.isOnline ? (
          <>
            ...
            >
              <div className="bg-white px-3 pt-3 border-b border-gray-100">
                <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-2">Accepting</p>
                <div className="grid grid-cols-3 gap-1.5 mb-3">
                  {serviceOptions.map(option => (
                    <button
                      key={option.value}
                      type="button"
                      onClick={() => option.value !== serviceType && serviceTypeMut.mutate(option.value)}
                      disabled={serviceTypeMut.isPending}
                      className={`h-9 rounded-xl text-xs font-extrabold transition-all disabled:opacity-60 ${
