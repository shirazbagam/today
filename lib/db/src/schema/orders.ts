...
export const ordersTable = pgTable("orders", {
  ...
  index("orders_payment_method_idx").on(t.paymentMethod),
...
export const orderStatusHistoryTable = pgTable("order_status_history", {
  ...
  changedByRole: text("changed_by_role").notNull().default("system"),
  note: text("note"),
  createdAt: timestamp("created_at").notNull().defaultNow(),
}, (t) => [
...
export const insertOrderStatusHistorySchema = createInsertSchema(orderStatusHistoryTable).omit({ createdAt: true });
