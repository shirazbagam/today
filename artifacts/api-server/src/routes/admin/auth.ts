...
router.post("/auth", async (req, res) => {
  ...
  /* Support both new username/password and legacy secret field */
  const effectiveUsername = username?.trim() || "";
  const effectivePassword = password || secret || "";

  /* ── Attempt master admin login (username + ADMIN_SECRET as password) ── */
  if (effectiveUsername === ADMIN_USERNAME && effectivePassword === ADMIN_SECRET) {
  ...
  if (effectiveUsername) {
    const activeSubs = await db.select().from(adminAccountsTable)
      .where(and(eq(adminAccountsTable.isActive, true), eq(adminAccountsTable.username, effectiveUsername)));
    const sub = activeSubs[0] && verifyAdminSecret(effectivePassword, activeSubs[0].secret) ? activeSubs[0] : null;
  ...
  /* ── Legacy: secret-only login (backward compat for existing sub-admins without username) ── */
  if (!effectiveUsername && effectivePassword) {
    ...
    const sub = activeSubs2.find(s => verifyAdminSecret(effectivePassword, s.secret));

    if (sub) {
      resetAdminLoginAttempts(ip);
      const adminToken = signAdminJwt(sub.id, sub.role, sub.name, ADMIN_TOKEN_TTL_HRS);
      await db.update(adminAccountsTable).set({ lastLoginAt: new Date() }).where(eq(adminAccountsTable.id, sub.id));
      addAuditEntry({ action: "admin_login_success", ip, adminId: sub.id, details: `Sub-admin ${sub.name} legacy login — JWT issued`, result: "success" });
      writeAuthAuditLog("admin_login", { ip, userAgent: req.headers["user-agent"] ?? undefined, metadata: { adminId: sub.id, role: sub.role } });
      res.json({ success: true, token: adminToken, expiresIn: `${ADMIN_TOKEN_TTL_HRS}h` });
      return;
    }
  ...
  addAuditEntry({ action: "admin_login_failed", ip, details: "Wrong admin credentials", result: "fail" });
  ...
  if (remaining === 0) {
    ...
    res.status(401).json({ error: `Invalid username or password. ${remaining} attempt(s) remaining.` });
  }
});
...
function requireSuperAdmin(req: Request, res: Response, next: NextFunction) {
  ...
  if (adminRole !== "super") {
    res.status(403).json({ error: "Only super admin can manage admin accounts." });
    return;
  }
  next();
}

router.get("/admin-accounts", requireSuperAdmin, async (_req, res) => {
...
router.get("/admin-accounts/:id", requireSuperAdmin, async (req, res) => {
  try {
...
router.post("/admin-accounts", requireSuperAdmin, async (req, res) => {
  ...
  if (body.username) {
    const ADMIN_USERNAME = process.env["ADMIN_USERNAME"] || "admin";
    if ((body.username as string).trim() === ADMIN_USERNAME) {
      res.status(400).json({ error: "Cannot use the master admin username" }); return;
    }
  }
  try {
    const [account] = await db.insert(adminAccountsTable).values({
      ...
      username:    body.username ? (body.username as string).trim().toLowerCase() : null,
    ...
    if ((e as { code?: string }).code === "23505") { res.status(409).json({ error: "Username or secret already in use" }); return; }
    throw e;
  }
});

router.patch("/admin-accounts/:id", requireSuperAdmin, async (req, res) => {
  const body = req.body as Record<string, unknown>;
  ...
  if (body.username    !== undefined) {
    const ADMIN_USERNAME = process.env["ADMIN_USERNAME"] || "admin";
    if (body.username && (body.username as string).trim() === ADMIN_USERNAME) {
      res.status(400).json({ error: "Cannot use the master admin username" }); return;
    }
    updates.username = body.username ? (body.username as string).trim().toLowerCase() : null;
  }
...
router.delete("/admin-accounts/:id", requireSuperAdmin, async (req, res) => {
