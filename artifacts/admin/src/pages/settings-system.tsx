...
export function SystemSection() {
  ...
  useEffect(() => {
    ...
    apiFetch("/snapshots").then(data => {
      ...
      console.error("[Settings System] Failed to load snapshots:", error);
      // Continue without previous undo snapshots
    });
  }, []);
  ...
  const handleDismissUndo = async (id: string) => {
    try { await apiFetch(`/snapshots/${id}`, { method: "DELETE" }); }
    catch (error) {
      console.error("[Settings System] Failed to delete snapshot:", error);
      // Continue - snapshot will expire anyway
    }
