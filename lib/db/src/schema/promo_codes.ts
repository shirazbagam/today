...
export const promoCodesTable = pgTable("promo_codes", {
  ...
  uniqueIndex("promo_codes_global_code_unique").on(t.code).where(sql`${t.vendorId} IS NULL`),
  uniqueIndex("promo_codes_vendor_code_unique").on(t.code, t.vendorId).where(sql`${t.vendorId} IS NOT NULL`),
]);
