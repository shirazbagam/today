...
import { usersTable, vendorProfilesTable, riderProfilesTable, ordersTable, orderStatusHistoryTable, orderItemsTable, productsTable, promoCodesTable, walletTransactionsTable, notificationsTable, reviewsTable, liveLocationsTable, deliveryWhitelistTable, deliveryAccessRequestsTable, bulkUploadLogsTable } from "@workspace/db/schema";
...
import { getIO, emitOrderStatusEvent, emitRiderNewRequest, emitNotificationNew } from "../lib/socketio.js";
  ...
  if (status === "cancelled" && order.paymentMethod === "wallet") {
    ...
    const txResult = await db.transaction(async (tx) => {
      ...
      await tx.insert(orderStatusHistoryTable).values({
        id: generateId(), orderId, previousStatus: order.status, status,
        changedByUserId: vendorId, changedByRole: "vendor",
      });
      return result[0];
    }).catch((err: Error) => {
    ...
    const result = await db.transaction(async (tx) => {
      const [row] = await tx.update(ordersTable)
        ...
        .where(and(eq(ordersTable.id, orderId), eq(ordersTable.vendorId, vendorId), eq(ordersTable.status, order.status)))
        .returning();
      if (!row) return null;
      await tx.insert(orderStatusHistoryTable).values({
        id: generateId(), orderId, previousStatus: order.status, status,
    ...
    if (!result) { sendError(res, "Order status has already been updated", 409); return; }
    updated = result;
  }

  if (msgs[status]) {
    const notifId = generateId();
    await db.insert(notificationsTable).values({ id: notifId, userId: order.userId, title: msgs[status]!.title, body: msgs[status]!.body, type: "order", icon: "bag-outline" }).catch(()=>{});
    emitNotificationNew(order.userId, { id: notifId, title: msgs[status]!.title, body: msgs[status]!.body, type: "order", icon: "bag-outline" });
  }
...
router.post("/promos", async (req, res) => {
  ...
  const discountPct = body.discountPct ?? (["percentage", "percent", "pct"].includes(discountType) ? body.discountValue : null);
  const discountFlat = body.discountFlat ?? (["fixed", "flat", "amount"].includes(discountType) ? body.discountValue : null);
  if (!normalizedCode || (!discountPct && !discountFlat)) {
  ...
  const [existing] = await db.select({ id: promoCodesTable.id }).from(promoCodesTable).where(and(eq(promoCodesTable.code, normalizedCode), eq(promoCodesTable.vendorId, vendorId))).limit(1);
  if (existing) { sendValidationError(res, "Promo code already exists"); return; }
  const [promo] = await db.insert(promoCodesTable).values({
    id: generateId(), code: normalizedCode,
    description: body.description || null,
    discountPct: discountPct ? String(discountPct) : null,
    discountFlat: discountFlat ? String(discountFlat) : null,
