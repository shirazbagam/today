...
import { ordersTable, orderStatusHistoryTable, orderItemsTable, usersTable, walletTransactionsTable, promoCodesTable, productsTable, liveLocationsTable, notificationsTable, vendorProfilesTable, supportTicketsTable } from "@workspace/db/schema";
...
import { getIO, emitRiderNewRequest, emitOrderStatusEvent } from "../lib/socketio.js";
...
const supportTicketSchema = z.object({
  category: z.enum(["support", "dispute", "refund", "delivery", "quality"]).optional(),
  subject: z.string().min(3).max(160).transform(stripHtml),
  message: z.string().min(5).max(2000).transform(stripHtml),
}).strip();
...
/* ── Promo code helper ─────────────────────────────────────────────────────── */
async function validatePromoCode(code: string, orderTotal: number, orderType: string, lang: import("@workspace/i18n").Language = "en", vendorId?: string | null): Promise<{
  ...
  const promo = promos.find(p => p.vendorId === vendorId) ?? promos.find(p => p.vendorId === null);

  if (!promo)                                          return { valid: false, discount: 0, discountType: null, error: t("apiErrPromoNotFound", lang) };
  if (promo.vendorId && promo.vendorId !== vendorId)    return { valid: false, discount: 0, discountType: null, error: t("apiErrPromoNotFound", lang) };
...
router.post("/", customerAuth, validateBody(placeOrderSchema), async (req, res) => {
  ...
  if (promoCode) {
    const promoResult = await validatePromoCode(promoCode, itemsTotal, type ?? "mart", lang, resolvedVendorId);
...
router.post("/:id/support-ticket", customerAuth, validateBody(supportTicketSchema), async (req, res) => {
  const orderId = req.params.id;
  const userId = req.customerId!;
  const { category = "support", subject, message } = req.body;
  const [order] = await db.select({ id: ordersTable.id, userId: ordersTable.userId }).from(ordersTable)
  ...
  const [ticket] = await db.insert(supportTicketsTable).values({
    ...
    subject,
    message,
    priority: category === "dispute" || category === "refund" ? "high" : "normal",
  }).returning();
  addAuditEntry({
    action: "support_ticket_create",
    ip: getClientIp(req),
    details: `Customer [${userId}] opened ${category} ticket for order ${orderId}`,
    result: "success",
  });
