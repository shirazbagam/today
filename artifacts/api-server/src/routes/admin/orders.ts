...
import {
  ...
  notificationsTable,
  ordersTable, orderStatusHistoryTable, orderItemsTable, pharmacyOrdersTable, parcelBookingsTable, ridesTable, rideBidsTable, supportTicketsTable,
...
import { getIO, emitOrderStatusEvent } from "../../lib/socketio.js";
  ...
  if (status === "cancelled" && preOrder.paymentMethod === "wallet" && !preOrder.refundedAt) {
    ...
    const txResult = await db.transaction(async (tx) => {
      ...
      await tx.insert(orderStatusHistoryTable).values({
        ...
        status,
        changedByUserId: (req as AdminRequest).adminId ?? null,
        changedByRole: "admin",
      });
      return result[0];
    }).catch((err: Error) => {
...
router.get("/support-tickets", async (req, res) => {
  ...
  const limit = Math.min(200, Math.max(1, parseInt(req.query?.limit as string) || 50));
  const offset = (page - 1) * limit;
  const whereClause = status && status !== "all" ? eq(supportTicketsTable.status, status) : undefined;
...
router.patch("/support-tickets/:id", validateParams(idParamSchema), async (req, res) => {
  const status = typeof req.body?.status === "string" ? req.body.status : undefined;
  const resolution = typeof req.body?.resolution === "string" ? req.body.resolution.slice(0, 2000) : undefined;
  if (status && !["open", "in_progress", "resolved", "closed"].includes(status)) {
    sendValidationError(res, "Invalid ticket status"); return;
  }
  ...
  const [ticket] = await db.update(supportTicketsTable)
    ...
    .returning();
  if (!ticket) { sendNotFound(res, "Support ticket not found"); return; }
  addAuditEntry({
    action: "support_ticket_update",
    adminId: (req as AdminRequest).adminId,
    ip: getClientIp(req),
    details: `Support ticket ${ticket.id} updated${status ? ` to ${status}` : ""}`,
    result: "success",
  });
