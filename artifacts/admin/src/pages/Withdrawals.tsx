...
function ApproveModal({ w, onClose }: { w: Withdrawal; onClose: () => void }) {
  ...
  const handleApprove = () => {
    ...
    approve.mutate({ id: w.id, refNo: refNo.trim(), note: note.trim() || undefined }, {
      ...
      onError:   (e: any) => toast({ title: "Withdrawal Approved Failed", description: e.message, variant: "destructive" }),
    });
  };
...
function RejectModal({ w, onClose }: { w: Withdrawal; onClose: () => void }) {
  ...
  const handleReject = () => {
    ...
    reject.mutate({ id: w.id, reason: reason.trim() }, {
      ...
      onError: (e: any) => toast({ title: "Failed to Reject Withdrawal", description: e.message, variant: "destructive" }),
    });
  };
